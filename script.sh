...
then 
   ...
   echo
   echo "${green} YOUR IP HAS ALLOWED ${norm}"
   echo " WELCOME USER "
   rm /etc/seveip
fi
# HELLO +GOOGLE
...
# COPY AND PASTE
cp /var/www/html/TCP.ovpn /var/www/html/BI.ovpn
sed -i '/remote/d' /var/www/html/BI.ovpn
cat <<EOF >>/var/www/html/BI.ovpn
remote "Your-Payload" 222
http-proxy $het 357
EOF
